...
#select * , row_number() over (partition by CardNumber order by update_date dec) rn from all where rn=1;
...
spark.sql("insert overwrite table prod.card_details select CardNumber ,CardType,full_name,contactnumber,emailid,address,city,country,issuedate,update_date,BilligDate,CardLimit,Active_flag from prod.card_details_temp")
...
spark.sql("insert overwrite table prod.debit_details select CardNumber,TransactionId,transactionDate,amountspend,category,debitstatus from prod.debit_details_temp")
...
spark.sql("insert overwrite table prod.credit_details select CardNumber,TransactionId,transactionDate,amountpaid,cardstatus,transaction_type_desc from prod.credit_details_temp")
